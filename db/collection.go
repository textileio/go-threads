...
func getInstanceID(t []byte) core.InstanceID {
	...
	if err := json.Unmarshal(t, partial); err != nil {
		log.Errorf("error when unmarshaling json instance: %v", err)
	}
	if partial.ID == nil {
		log.Error("invalid instance: doesn't have an _id attribute")
	}
	if *partial.ID != "" && !core.IsValidInstanceID(*partial.ID) {
		log.Error("invalid instance: invalid _id value")
	}
...
func setNewInstanceID(t []byte) (core.InstanceID, []byte) {
	...
	if err != nil {
		log.Errorf("error while automatically patching autogenerated _id: %v", err)
	}
