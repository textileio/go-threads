...
type service struct {
	...
	pullLock  sync.Mutex
	pullLocks map[core.ID]chan struct{}
}
...
func NewService(
	...
	t := &service{
		...
		ctx:        ctx,
		cancel:     cancel,
		pullLocks:  make(map[core.ID]chan struct{}),
	}
...
func (t *service) AddThread(
	...
	if err = t.store.AddThread(core.Info{
		ID:        id,
...
func (r *record) ThreadID() core.ID {
	return r.threadID
}
...
func (t *service) Subscribe(opts ...core.SubOption) core.Subscription {
	...
	filter := make(map[core.ID]struct{})
