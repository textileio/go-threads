...
func main() {
	...
	{
		var books []*book
		err := model.Find(&books, db.Where("Author").Eq("Author1"))
		checkErr(err)
		if len(books) != 2 {
	...
	{
		var books []*book
		err := model.Find(&books, db.Where("Meta.TotalReads").Eq(100))
		checkErr(err)
		if len(books) != 1 {
	...
	{
		var books []*book
		err := model.Find(&books, db.Where("Author").Eq("Author1").And("Title").Eq("Title2"))
		checkErr(err)
		if len(books) != 1 {
	...
	{
		var books []*book
		err := model.Find(&books, db.Where("Author").Eq("Author1").Or(db.Where("Author").Eq("Author2")))
		checkErr(err)
		if len(books) != 3 {
	...
	// Sorted query
	{
		var books []*book
		// Ascending
		err := model.Find(&books, db.Where("Author").Eq("Author1").OrderBy("Meta.TotalReads"))
		checkErr(err)
		if books[0].Meta.TotalReads != 100 || books[1].Meta.TotalReads != 150 {
			panic("books aren't ordered asc correctly")
		}
		// Descending
		err = model.Find(&books, db.Where("Author").Eq("Author1").OrderByDesc("Meta.TotalReads"))
		checkErr(err)
	...
	{
		var books []*book
		err := model.Find(&books, db.Where("Title").Eq("Title3"))
		checkErr(err)
		...
		// Delete it
		err = model.Find(&books, db.Where("Title").Eq("ModifiedTitle"))
		checkErr(err)
		if len(books) != 1 {
