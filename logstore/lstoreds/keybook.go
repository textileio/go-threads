...
// of (service.ID, peer.ID) pairs with durable guarantees by store.
func NewKeyBook(store ds.Datastore) (core.KeyBook, error) {
	return &dsKeyBook{ds: store}, nil
}

// PubKey returns the public key of (service.ID, peer.ID). The implementation
...
func (kb *dsKeyBook) PubKey(t service.ID, p peer.ID) (ic.PubKey, error) {
...
func (kb *dsKeyBook) AddPubKey(t service.ID, p peer.ID, pk ic.PubKey) error {
	if pk == nil {
...
// PrivKey returns the private key of (service.ID, peer.ID). If not private key
// is stored, returns nil.
func (kb *dsKeyBook) PrivKey(t service.ID, p peer.ID) (ic.PrivKey, error) {
...
func (kb *dsKeyBook) AddPrivKey(t service.ID, p peer.ID, sk ic.PrivKey) error {
	if sk == nil {
...
// ReadKey returns the read-key associated with service.ID thread.
// In case it doesn't exist, it will return nil.
func (kb *dsKeyBook) ReadKey(t service.ID) (*sym.Key, error) {
...
func (kb *dsKeyBook) AddReadKey(t service.ID, rk *sym.Key) error {
	if rk == nil {
...
// FollowKey returns the follow-key associated with service.ID service.
// In case it doesn't exist, it will return nil.
func (kb *dsKeyBook) FollowKey(t service.ID) (*sym.Key, error) {
...
func (kb *dsKeyBook) AddFollowKey(t service.ID, fk *sym.Key) error {
	if fk == nil {
...
func (kb *dsKeyBook) LogsWithKeys(t service.ID) (peer.IDSlice, error) {
...
func (kb *dsKeyBook) ThreadsFromKeys() (service.IDSlice, error) {
