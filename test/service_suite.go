...
var serviceSuite = []map[string]func(core.Service, core.Service) func(*testing.T){
	{"AddPull": testAddPull},
	{"AddPeer": testAddPeer},
...
func newService(t *testing.T, hostAddr ma.Multiaddr, hostProxyAddr ma.Multiaddr) core.Service {
	...
	ts, err := service.NewService(
		...
		tstore.NewLogstore(),
		service.Config{ProxyAddr: hostProxyAddr, Debug: true})
	check(t, err)
	return ts
}

func testAddPull(ts1, _ core.Service) func(t *testing.T) {
...
func testAddPeer(ts1, ts2 core.Service) func(t *testing.T) {
	return func(t *testing.T) {
		...
		info, err := ts2.AddThread(ctx, addr, core.FollowKey(th.FollowKey), core.ReadKey(th.ReadKey))
		check(t, err)
		if info.Logs.Len() != 1 {
...
func testClose(ts1, ts2 core.Service) func(t *testing.T) {
	return func(t *testing.T) {
		err := ts1.Close()
		check(t, err)
		err = ts2.Close()
