...
for proto_path in "${PROTO_PATHS[@]}"; do
  echo "${proto_path}"
  for dir in $(find "${proto_path}" -type f ! -path './net/pb/*' ! -path './buildtools/*' ! -path '*/node_modules/*' -name '*.proto' -print0  | xargs -0 -n1 dirname | sort | uniq); do
    echo protoc "${PROTOC_FLAGS[@]}" "$(find "${dir}" -name '*.proto' ! -path './net/pb/*' ! -path '*/node_modules/*')"
    ./buildtools/protoc/bin/protoc "${PROTOC_FLAGS[@]}" "$(find "${dir}" -name '*.proto' ! -path './net/pb/*' ! -path '*/node_modules/*')"
  done
done
